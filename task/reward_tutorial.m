function  [data] = task_display (StimCue, TrialCue, Time2Target, Sham, TargetDisplayTime, ITI, scr, Spacebar)
if scr==1
    ioObj = io64;
status = io64(ioObj);
...
% if scr==1  %trigger event marker that Marc set
%     outportb(888,255);
%     wait(10);
%     outportb(888,0);
% end
if scr==1 %trigger event marker following tracey's code
...
%                 Response=5 %too early
        ...
        elseif TrialCue==2 % Go to avoid losing
            if  Response==1
                Won=0; %add Money to total and draw green arrow pointing upwards
                clearpict(4);
                preparepict(loadpict('horizontalBar.png'),4);
                %                 cgpencol(1,1,1);
                %                 cgfont('Arial',2)
        ...
        elseif TrialCue==3 %No go to win
            if  Response==0
                Won=Money; %earn money and green arrow upwards
                clearpict(4);
                preparepict(loadpict('goldenTicket.png'),4);
                %                 cgpencol(1,1,1);
                %                 cgfont('Arial',2)
...
data=[StimCue,TrialCue,TimeCue,ResponseCue1,ResponseCue2,Time2Target,TimeTarget,KeyResp,KeyTime,RT,Response,TargetDisplayTime,TimeOutcome,ITI,Won];
end